-- auth_manager.lua - 不使用resty.http的版本
local cjson = require "cjson"
-- 注释掉resty.http，使用nginx内置功能
...
-- 使用nginx.location.capture获取OAuth2 Token
local function get_oauth2_token(service_account)
    ...
    if config.should_test_output("oauth_process") then
        ngx.log(ngx.INFO, "[TEST] Requesting OAuth2 token from Google")
    end

    -- 使用nginx的proxy_pass功能或者返回模拟token（用于测试）
    -- 在生产环境中，这里需要实现实际的HTTP请求
    -- 目前返回一个模拟的token用于测试
    local token_data = {
        access_token = "mock_access_token_" .. ngx.time(),
        token_type = "Bearer",
        expires_in = 3600,
    ...
    if config.should_test_output("oauth_process") then
        ngx.log(ngx.INFO, "[TEST] Mock token generated for testing")
    end
