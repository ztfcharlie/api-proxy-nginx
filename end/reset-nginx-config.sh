...
echo "1. Checking current broken config..."
docker-compose exec api-proxy-nginx tail -5 /usr/local/openresty/nginx/conf/conf.d/gemini-proxy.conf

echo ""
echo "2. Stopping nginx to reset config safely..."
docker-compose stop api-proxy-nginx

echo ""
echo "3. The config file is mounted from host, let's check the host file..."
tail -5 nginx/conf.d/gemini-proxy.conf

echo ""
echo "4. Checking if the host config file is correct..."
# 检查宿主机上的配置文件是否正确
if grep -q "unexpected" nginx/conf.d/gemini-proxy.conf; then
    echo "Host config file is also corrupted, need to restore from backup"
else
    echo "Host config file looks OK"
fi

echo ""
echo "5. Let's restore the original gemini-proxy.conf..."

# 重新创建正确的配置文件
cat > nginx/conf.d/gemini-proxy.conf << 'EOF'
server {
    listen 8080;
    server_name _;
...
echo "✓ Restored original gemini-proxy.conf"

echo ""
echo "6. Starting nginx with clean config..."
docker-compose start api-proxy-nginx
...
echo ""
echo "9. Testing basic endpoints..."
echo "Health check:"
...
echo ""
echo "10. Checking nginx logs for any errors..."
docker logs api-proxy-nginx --tail 10

echo ""
echo "=== Configuration Reset Complete ==="
echo ""
echo "Now we can proceed to test resty.http and restore full auth_manager.lua"