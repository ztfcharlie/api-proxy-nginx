...
init_env() {
    ...
    directories=(
        ...
        "data/users"
        "data/users_client_tokens"
        "data/channel_secret_keys"
        "data/channel_oauth_caches"
        "mysql-data"
        "mysql-files"
        "nodejs/logs"
        "nodejs/tmp"
...
case "$1" in
    ...
    init_database)
        ...
        ;;
    help|*)
        echo "用法: $0 {init|start|stop|restart|status|logs|backupdb|init_database}"
        echo "----------------------------------------------------------------"
        ...
        echo "  init_database [file] - 从 SQL 文件恢复数据库"
        echo "                         示例: ./run.sh init_database database/backups/backup_2023.sql"
        echo "----------------------------------------------------------------"
        exit 1
        ;;
esac